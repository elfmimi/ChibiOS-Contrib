...
/**
 * @file    hal_pal_lld.h
 * @brief   NUC123 PAL subsystem low level driver header.
 *
 * @addtogroup PAL
 * @{
 */
 ...
 * @note    In this driver the port number is encoded in the upper 4 bits which
 *          are zero by default in the GPIO->DOUT register
 */
#define PAL_LINE(port, pad)                                                 \
  ((ioline_t)((uint32_t)(((port - GPIO_BASE) / 0x40) << 4) | ((uint32_t)(pad)))

/**
 * @brief   Decodes a port identifier from a line identifier.
 */
#define PAL_PORT(line)                                                      \
  ((GPIO_T *)(((line >> 4) * 0x40) + GPIO_BASE))
