...
void config_frequency(I2CDriver *i2cp) {
  ...
  if (i2cp->config != NULL)
    divisor = (NUC123_HCLK * 10 / (i2cp->config->clock * 4) + 5) / 10 - 1;
  else
    divisor = NUC123_HCLK / 4 / 100000 - 1;
...
static void serve_interrupt(I2CDriver *i2cp) {
  ...
  uint8_t status = (uint8_t)(i2c->I2CSTATUS);

  if (status == 0x08 || status == 0x10) {
    if (i2cp->intstate == STATE_STOP) {
      // i2c->I2CON = ((i2c->I2CON & ~I2C_I2CON_STA_STO_SI_AA) | I2C_I2CON_SI);
      i2c->I2CON = (i2c->I2CON & ~I2C_I2CON_EI_Msk);
      return;
    }
    i2cp->is_master = 1;
  }
  ...
  if (i2cp->is_master) {
	...
	if (i2cp->txbuf != NULL && i2cp->txidx < i2cp->txbytes) {
	  ...
	  i2c->I2CDAT = i2cp->txbuf[i2cp->txidx++];
	  // i2c->I2CON |= I2C_I2CON_SI;
	  I2C_SET_I2CON(i2c, I2C_I2CON_SI);
	} else {
	  ...
	  if (i2cp->rxbuf != NULL && i2cp->rxbytes != 0) {
	    ...
	    i2c->I2CON |= I2C_I2CON_STA_SI; // should we use Repeat-START ?
	  } else {
...
_i2c_wakeup_isr(i2cp);
	...
	i2cp->rxbuf[i2cp->rxidx++] = i2c->I2CDAT;
...
void i2c_lld_start(I2CDriver *i2cp) {
  ...
  i2cp->i2c->I2CTOC =  I2C_I2CTOC_TIF_Msk;
...
static inline msg_t _i2c_txrx_timeout(I2CDriver *i2cp, i2caddr_t addr,
  ...
  if(i2cp->is_master) {
    ...
    while((uint8_t)(i2cp->i2c->I2CSTATUS) == 0xF8);
    ...
    // i2cp->i2c->I2CON |=  I2C_I2CON_STA_SI;
    uint8_t status = (uint8_t)(i2cp->i2c->I2CSTATUS);
    if (status == 0x08 || status == 0x10) {
      i2cp->is_master = 1;
      // uint8_t op = (i2cp->intstate == STATE_SEND) ? 0 : 1;
      ...
      i2cp->i2c->I2CON = (i2cp->i2c->I2CON | I2C_I2CON_EI_Msk);
    } else {
