 ...
 */
#if (NUC123_USB_USE_USB0 == TRUE) || defined(__DOXYGEN__)
#ifndef USBD_INTSTS_EPEVT_Pos
#define USBD_INTSTS_EPEVT_Pos USBD_INTSTS_EPEVT0_Pos
#define USBD_INTSTS_EPEVT_Msk (0xff << USBD_INTSTS_EPEVT0_Pos)
#endif
USBDriver USBD1;
#endif
...
void usb_lld_start(USBDriver *usbp) {
  if (usbp->state == USB_STOP) {
    ...
    if (&USBD1 == usbp) {
      ...
      nvicEnableVector(USBD_IRQn, NUC123_USB_USB0_IRQ_PRIORITY);
  ...
  if (epcp->in_state != NULL) {
    ...
    if (epcp->ep_mode == USB_EP_MODE_TYPE_CTRL) {
      ...
      USBD->EP[hwep].CFG = (ep << USBD_CFG_EP_NUM_Pos) | (2 << USBD_CFG_STATE_Pos);
          epcp->in_state->dsq = !0; // without this the very first packet we send out will be ignored
    }
