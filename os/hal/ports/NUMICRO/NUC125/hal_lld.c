...
void SystemCoreClockUpdate(void) /* Get Core Clock Frequency      */
{
  ...
  if (PllReg & (CLK_PLLCON_PD_Msk | CLK_PLLCON_OE_Msk)) {
    ...
    if (PllReg & 0x00080000ul) {
      pllFIN = __HIRC/2; /* Use HSI for PLL clock */
    } else {
#ifdef NUC123_HSECLK
      pllFIN = NUC123_HSECLK; /* Use HXT for PLL clock */
#else
      pllFIN = 12000000; /* This won't happen. */
#endif
    }
...
#else
    clkFreq = 12000000; /* This won't happen. */
#endif
    break;
  case 1: /* External LF Xtal */
    clkFreq = __LXT;
    break;
  ...
  case 5: /* PLL clock / 2 */
    clkFreq = PllClock >> 1;
    break;
  case 4: /* Internal 48MHz */
    clkFreq = __HIRC;
    break;
  case 7: /* Internal 48MHz / 2 */
    clkFreq = __HIRC >> 1;
    break;
  default:
    clkFreq = 0;
...
static void set_HCLK(uint32_t clkSource, uint32_t clkDivider)
{
  ...
  /* Use __HIRC/2 as HCLK, temporarily */
  CLK->CLKSEL0 =
      (CLK->CLKSEL0 & (~CLK_CLKSEL0_HCLK_S_Msk)) | NUC125_HCLKSRC_HSI_2;
...
static uint32_t enable_pll(uint32_t pllSrc, uint32_t pllFreq)
{
  ...
  case NUC123_PLLSRC_HSE:
    NR      = 2;
#ifdef NUC123_HSECLK
    clkCalc = NUC123_HSECLK; /* Use HXT for PLL clock */
#else
    clkCalc = 12000000; /* This won't happen. */
#endif
    break;
  case NUC123_PLLSRC_HSI:
    NR      = 4;
...
static uint32_t set_core_clock(uint32_t clkCore)
{
  ...
  if (CLK->CLKSTATUS & CLK_CLKSTATUS_XTL12M_STB_Msk) {
    ...
    clkCore = enable_pll(NUC123_PLLSRC_HSE, NUC123_PLLCLK);
  } else {
    /* Use __HIRC as PLL source */
    clkCore = enable_pll(NUC123_PLLSRC_HSI, NUC123_PLLCLK);
  ...
  set_HCLK(NUC125_HCLKSRC_PLL, CLK_CLKDIV_HCLK(NUC123_PLLCLK / NUC123_HCLK));
  ...
  return clkCore / (NUC123_PLLCLK / NUC123_HCLK);
}
...
void NUC123_clock_init(void)
{
  ...
  CLK->PWRCON |= CLK_PWRCON_OSC22M_EN_Msk;
  wait_for_clock_ready(CLK_CLKSTATUS_OSC22M_STB_Msk);
  set_HCLK(NUC125_HCLKSRC_HSI_2, CLK_CLKDIV_HCLK(1));
  ...
  CLK->PWRCON |= CLK_PWRCON_XTL12M_EN_Msk;
  wait_for_clock_ready(CLK_CLKSTATUS_XTL12M_STB_Msk);
  set_HCLK(NUC123_HCLKSRC_HSE, CLK_CLKDIV_HCLK(1));
#else
  // Enable Crystal-less
  SYS->IRCTCTL = 0x04C2;
  ...
  // Select PLL for USBD_CLK
  // M32(0x50000234) |= 0x0100; // CLKSEL3.USBDSEL = CLKSEL3_USBDSEL_PLL
#endif /* NUC123_PLL_ENABLED */

#if 1
  // Output HCLK to CLKO and map to PB.12
  CLK->CLKSEL2 = 8; // CLKO = HCLK
  CLK->FRQDIV = 0x30; // CLKOEN = 1  DIV1EN = 1
  CLK->APBCLK |= 0x40; // CLKOCKEN = 1
  SYS->GPB_MFPH = (SYS->GPB_MFPH & ~0x000F0000) | 0x00020000;
#endif
